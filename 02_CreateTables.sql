...
/****** Object:  Table [dbo].[assignments]    Script Date: 07/13/2014 11:59:35 ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS ( SELECT  *
                FROM    sys.objects
                WHERE   object_id = OBJECT_ID(N'[dbo].[Assignments]')
                        AND type IN ( N'U' ) ) 
    BEGIN
		...
		(
			...
			CONSTRAINT [PK_Assignments_1] PRIMARY KEY CLUSTERED ( [Id] ASC )
                WITH ( PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON,
                       ALLOW_PAGE_LOCKS = ON ) ON [PRIMARY]
		)
		ON  [PRIMARY]
